...
export const exportCalls = asyncHandler(async (req, res) => {
  ...
  const calls = await prisma.call.findMany({
    ...
    take: 50000,
    select: {
      callId: true, phoneNumber: true, customerName: true, direction: true, callStatus: true,
      disposition: true, remark: true, durationSeconds: true, createdAt: true,
  ...
  const rows = calls.map((c) => ({
    ...
    direction: titleCaseEnum(c.direction),
    agent: c.user?.name || '',
    client: c.client?.name || c.customerName || '',
    phone: c.phoneNumber,
